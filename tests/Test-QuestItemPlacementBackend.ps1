...
$seederRuntimePath = Join-Path $repoRoot `
    'src\Data\Scripts\mods\dpevidenceseeder.lua'
...
$bindings = [pscustomobject]@{
    settlements = @(
        ...
        [pscustomobject]@{
            ...
            region = 'kutnohorsko'
            settlement = 'sasau'
            nativeVariantIds = @('fixture.sasau')
            roles = [pscustomobject]@{
                document = [pscustomobject]@{
                    documentGuid = '73762008-de9b-4c42-b509-235e63e60840'
                    containerGuid = 'aaaaaaaa-bbbb-cccc'
                }
            }
        },
        [pscustomobject]@{
            ...
            nativeVariantIds = @('fixture.troskovice')
            roles = [pscustomobject]@{
                document = [pscustomobject]@{
                    documentGuid = '73762008-de9b-4c42-b509-235e63e60840'
...
Add-Result ($signals.Count -eq 4) `
    'quest documents emit one addressed signal per compatible regional stash'
Add-Result (
...
Add-Result (
    @($signals | Where-Object {
        ...
        $_.request_key -eq '277db45d-28ac-0286' -and
...
Add-Result (
    @($signals | Where-Object {
        $_.item_guid -eq '73762008-de9b-4c42-b509-235e63e60840' -and
        $_.request_key -eq 'aaaaaaaa-bbbb-cccc' -and
        @($_.stash_bindings).Count -eq 1 -and
        $_.stash_bindings[0].settlement -eq 'sasau'
    }).Count -eq 1
) 'same document class receives an independent addressable stash request'
Add-Result (
    @($signals | Where-Object {
        $_.item_guid -eq '73762008-de9b-4c42-b509-235e63e60840' -and
        $_.request_key -eq '042bf770-1c46-03bf' -and
        @($_.stash_bindings).Count -eq 1 -and
        $_.stash_bindings[0].region -eq 'trosecko' -and
        $_.stash_bindings[0].settlement -eq 'troskovice'
    }).Count -eq 1
) 'multi-region story emits the same quest document for Troskovice'
Add-Result (
    @($signals.signal_tag | Sort-Object -Unique).Count -eq $signals.Count -and
    (@($signals | ForEach-Object { [int]$_.signal_tag }) |
        Measure-Object -Minimum).Minimum -eq 240
) 'placement signals use a caller-owned non-overlapping tag range'
...
Add-Result (
    ([regex]::Matches($tagXml, '<buff_ai_tag ')).Count -eq 4
) 'quest-item request tags are generated once per concrete destination'
Add-Result (
    ([regex]::Matches($buffXml, '<buff ')).Count -eq 4 -and
...
Add-Result (
    ([regex]::Matches($nodes, '<AddQuestItem ')).Count -eq 2 -and
    $nodes.Contains(
    ...
    ([regex]::Matches($nodes, 'MethodName="CreateItems"')).Count -eq 1 -and
    ([regex]::Matches($nodes, '<UseBookTrigger ')).Count -eq 1
) 'addressed documents use AddQuestItem without duplicate read triggers'
Add-Result (
    $assets.Contains(
        '<StashAsset Name="DP_EvidenceStash_kutnohorsko_pritoky" />'
    ) -and
    $assets.Contains(
        '<StashAsset Name="DP_EvidenceStash_kutnohorsko_sasau" />'
    ) -and
    ([regex]::Matches($assets, '<StashAsset ')).Count -eq 2
...
Add-Result (
    ...
    $catalog.Contains('backend = "quest_effect_stash"') -and
    $catalog.Contains('requests =') -and
    $catalog.Contains('["277db45d-28ac-0286"]') -and
    $catalog.Contains('["aaaaaaaa-bbbb-cccc"]')
) 'Lua placement catalog maps one item class to addressed native requests'

$sharedContainerSignals = @(
    [ordered]@{
        item_guid = '73762008-de9b-4c42-b509-235e63e60840'
        request_key = '042bf770-1c46-03bf'
        buff_guid = '11111111-1111-1111-1111-111111111111'
        signal_tag = 310
        retention = 'case'
        backend = 'quest_effect_stash'
        read_context = 'dp_document_read_73762008'
    },
    [ordered]@{
        item_guid = 'd5833fd4-f7bf-4957-86f5-d661db38bcf3'
        request_key = '042bf770-1c46-03bf'
        buff_guid = '22222222-2222-2222-2222-222222222222'
        signal_tag = 311
        retention = 'case'
        backend = 'quest_effect_stash'
        read_context = 'dp_document_read_d5833fd4'
    }
)
$sharedContainerCatalog = ConvertTo-DpQuestItemPlacementCatalogLua `
    -Signals $sharedContainerSignals
$firstSharedItemBlock = [regex]::Match(
    $sharedContainerCatalog,
    '(?ms)^    \["73762008-de9b-4c42-b509-235e63e60840"\] = \{.*?^    \},$'
).Value
$secondSharedItemBlock = [regex]::Match(
    $sharedContainerCatalog,
    '(?ms)^    \["d5833fd4-f7bf-4957-86f5-d661db38bcf3"\] = \{.*?^    \},$'
).Value
Add-Result (
    -not [string]::IsNullOrWhiteSpace($firstSharedItemBlock) -and
    -not [string]::IsNullOrWhiteSpace($secondSharedItemBlock) -and
    $firstSharedItemBlock.Contains('signal_tag = 310') -and
    -not $firstSharedItemBlock.Contains('signal_tag = 311') -and
    $secondSharedItemBlock.Contains('signal_tag = 311') -and
    -not $secondSharedItemBlock.Contains('signal_tag = 310')
) 'catalog keeps separate native requests for two documents sharing one stash'
Add-Result (
...
$seederRuntime = if (Test-Path -LiteralPath $seederRuntimePath) {
    [System.IO.File]::ReadAllText($seederRuntimePath)
} else { '' }
...
Add-Result (
    ...
    $runtime.Contains('requestKey') -and
    $runtime.Contains('entry.requests') -and
...
Add-Result (
    $runtime.Contains('string.lower(tostring(itemGuid or ""))') -and
    $runtime.Contains('string.lower(tostring(requestKey or ""))')
) 'Lua resolves mixed-case item and destination GUIDs'
Add-Result (
    $runtime.Contains('DarkPassengerQuestItemPlacement.pendingRequests') -and
    $runtime.Contains('REISSUE_INTERVAL_SECONDS') -and
    $runtime.Contains('return false, "native_pending"')
) 'accepted native placement requests enter a throttled pending state'
Add-Result (
    $belongingsRuntime.Contains('local placed, reason = EnsurePlaced(generation)') -and
    $belongingsRuntime.Contains('return placed, reason') -and
    $seederRuntime -match (
        'local placed, placementReason\s*=\s*' +
        'DarkPassengerBelongings\.EnsurePlaced\('
    ) -and
    -not $seederRuntime.Contains('return false, "document_deferred"')
) 'placement reason propagates across quest-item, belongings and seeder boundary'
Add-Result (
    $seederRuntime.Contains('lastDeferredByEvidence') -and
    $seederRuntime.Contains('LogDeferredOnce(')
) 'evidence polling logs a deferred state only when that state changes'
